...
const Profile = () => {
    ...
    try {
      ...
      if (
        ...
        (!clientProfile.coordinates ||
          clientProfile.coordinates.length === 0 ||
          (clientProfile.coordinates[0].lat === 0 && clientProfile.coordinates[0].lng === 0))
      ) {
        addressChanged = true;
      }
      ...
      if (addressChanged) {
        const { ward, coordinates: fetchedCoordinates } = await getWardAndCoordinates(
          ...
          {dietaryOptions.map((option: DietaryOption) => (
            <FormControlLabel
              ...
              control={
                <Checkbox
                  checked={Boolean(
                    clientProfile.deliveryDetails?.dietaryRestrictions?.[option.name]
                  )}
  ...
  return (
    <Box
      ...
      sx={{ backgroundColor: "#f8f9fa", minHeight: "100vh", pb: 4 }}
    >
      {showSavePopup && (
        <SaveNotification>
